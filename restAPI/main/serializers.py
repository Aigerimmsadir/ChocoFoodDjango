...
class RestaurantReviewModelSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        restReview = RestaurantReview.objects.create(restaurant = restaurant,review=review,**validated_data)
        print(validated_data)
        restReview.save()
        return restReview
    def update(self, instance, validated_data):
...
class DishReviewModelSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        dishReview = DishReview.objects.create(dish=dish,review=review,**validated_data)
...
class OrderModelSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = [ 'date', 'price', 'restaurant','owner']

    def create(self, validated_data):
        tracks_data = validated_data.pop('restaurant')
        rest = Restaurant.objects.get(name=tracks_data.get('name'))

        order = Order.objects.create(restaurant = rest,**validated_data)
